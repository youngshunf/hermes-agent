            ...
            try:
                ...
                if retries > _MAX_RECONNECT_RETRIES:
                    logger.warning(
                        ...
                        self.name, _MAX_RECONNECT_RETRIES, detail,
                    )
                    ...
                    # half-open probe, OAuth recovery, or a manual /mcp refresh
                ...
                # HuanXing: interruptible backoff so a manual /v1/mcp/reconnect
                # (or daemon-driven recovery) wakes the in-budget reconnect
                # sleep immediately and resets the retry counter.
                woken = await self._sleep_or_wake(backoff)
                backoff = 1.0 if woken else min(backoff * 2, _MAX_BACKOFF_SECONDS)
                if woken:
                    retries = 0
