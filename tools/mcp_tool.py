...
class MCPServerTask:
    ...
    async def _discover_tools(self):
        ...
        if self.session is None:
            return
        # 能走到这里说明会话已建立、可发现工具 —— 清掉上一轮记录的连接错误，
        # 让 get_mcp_status 反映「已恢复」（唤星补丁）。
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                self.session = None
                # 唤星补丁：把连接失败原因记进 _server_last_error，供
                # get_mcp_status 在服务器掉线时告诉主人「为什么断了」。
                # 下一轮成功发现工具时由 _discover_tools 清除。
                _record_server_error(self.name, _format_connect_error(exc))
...
def get_mcp_status() -> List[dict]:
    ...
    for name, cfg in configured.items():
        ...
        # 唤星补丁：统一算出「是否仍在后台重连」「断路器是否打开」「最近一次连接错误」，
        # 供分身详情页 MCP 状态卡如实展示掉线原因（对齐 test_mcp_huanxing_resilience）。
        _bg_task = getattr(server, "_task", None) if server else None
        is_retrying = bool(_bg_task is not None and not _bg_task.done())
        circuit_open = _server_error_counts.get(name, 0) >= _CIRCUIT_BREAKER_THRESHOLD
        last_error = _server_last_error.get(name)
...
def reconnect_mcp_servers(name: Optional[str] = None) -> List[dict]:
    """手动 / 守护进程驱动的 MCP 服务器重连（唤星补丁）。

    拆掉现有连接，并**从磁盘最新配置**重建 —— 这样能拾取轮换后的凭据
    （如刷新过的 Agent MCP key），同时把该服务器的错误计数、断路器、
    最近一次连接错误全部清零。``name=None`` 表示重连全部已配置服务器。

    由 gateway 的 ``POST /v1/mcp/reconnect`` 与原生 ``reconnect_tools`` 工具
    调用（daemon 侧 401 自愈 / 主人在分身详情页手动点「重连」都会走到这里）。

    返回：重建后的 :func:`get_mcp_status` 快照。
    """
    if not _MCP_AVAILABLE:
        return get_mcp_status()

    fresh = _load_mcp_config() or {}
    # 决定重连目标：指定了 name 就只重连它（且必须在最新配置里，否则无从重建）；
    # 否则重连全部已配置服务器。
    if name is not None:
        if name not in fresh:
            # 配置里没有这个名字 —— 无可重建，直接返回现状（幂等 no-op）。
            return get_mcp_status()
        targets = [name]
    else:
        targets = list(fresh.keys())

    _ensure_mcp_loop()

    to_build: Dict[str, dict] = {}
    for t in targets:
        # 先从注册表摘掉旧连接，再拆除，避免重建期间有半死连接被复用。
        with _lock:
            old = _servers.pop(t, None)
        if old is not None:
            try:
                _run_on_mcp_loop(lambda o=old: o.shutdown())
            except Exception as exc:
                logger.warning(
                    "MCP server '%s': shutdown during reconnect failed: %s", t, exc
                )
        # 重置断路器与记录的连接错误 —— 这是一次全新的尝试。
        _reset_server_error(t)
        _record_server_error(t, None)
        to_build[t] = fresh[t]

    if to_build:
        register_mcp_servers(to_build)
